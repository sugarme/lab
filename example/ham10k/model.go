...
// NewSelfAttention creates new SelfAttention struct.
func NewSelfAttention(p *nn.Path, inDim int64) *SelfAttention{
	outDim := inDim/int64(8)	
	q := nn.NewConv2D(p, inDim, outDim, 1, nn.DefaultConv2DConfig())
	k := nn.NewConv2D(p, inDim, outDim, 1, nn.DefaultConv2DConfig())
	v := nn.NewConv2D(p, inDim, inDim, 1, nn.DefaultConv2DConfig())

	// self.gamma = nn.Parameter(torch.zeros(1))
	gamma := ts.MustZeros([]int64{1}, gotch.Float, gotch.CPU.CudaIfAvailable())
...
// Forward forward past through SelfAttention.
// 
// x: input feature maps (Batch x Channel x Width x Height)
// returns:
// - out: self attention value + input feature
// - attn: attention map (weight)
func(sa *SelfAttention) ForwardT(x *ts.Tensor, train bool) (*ts.Tensor, *ts.Tensor){
	dim := x.MustSize()
	dtype := x.DType()
	m, c, w, h := dim[0], dim[1], dim[2], dim[3]
	
	q := sa.QueryConv.ForwardT(x, train).MustView([]int64{m, -1, w*h}, true).MustPermute([]int64{0, 2, 1}, true) // B x C x (N)
	k := sa.KeyConv.ForwardT(x, train).MustView([]int64{m, -1, w*h}, true) // B x C x (W*H)
	energy := q.MustBmm(k, true) // transpose check

	attention := energy.MustSoftmax(-1, dtype, true)
	v := sa.ValueConv.ForwardT(x, train).MustView([]int64{m, -1, w*h}, true) // B x C x N

	out := v.MustBmm(attention, true).MustView([]int64{m, c, w, h}, true)

	outGamma := sa.Gamma.MustMm(out, false).MustAdd(x, true)
	return outGamma, attention
}
