...
func ExampleNew() {
	df := data.NewDataframe(
		data.NewSeries([]string{"b", "a"}, data.String, "COL.1"),
		data.NewSeries([]int{1, 2}, data.Int, "COL.2"),
		data.NewSeries([]float64{3.0, 4.0}, data.Float, "COL.3"),
	)
	fmt.Println(df)
...
func ExampleLoadRecords_options() {
	df := data.LoadRecords(
		...
		data.DetectTypes(false),
		data.DefaultType(data.Float),
		data.WithTypes(map[string]data.Type{
			"A": data.String,
			"D": data.Bool,
		}),
	)
...
`
	df := data.ReadCSV(strings.NewReader(csvStr))
	fmt.Println(df)
...
func ExampleReadJSON() {
	...
	df := data.ReadJSON(strings.NewReader(jsonStr))
	fmt.Println(df)
...
func ExampleDataFrame_Mutate() {
	...
	mut := df.Mutate(
		data.NewSeries([]string{"a", "b", "c", "d"}, data.String, "C"),
	)
	// Add a new column E
	mut2 := df.Mutate(
		data.NewSeries([]string{"a", "b", "c", "d"}, data.String, "E"),
	)
	fmt.Println(mut)
	fmt.Println(mut2)
