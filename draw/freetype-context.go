...
func NewGraphicContext(img draw.Image) *FTGraphicContext {
...
func NewGraphicContextWithPainter(img draw.Image, painter Painter) *FTGraphicContext {
...
func (gc *FTGraphicContext) GetDPI() int {
	return gc.DPI
}

// Clear fills the current canvas with a default transparent color
func (gc *FTGraphicContext) Clear() {
...
func (gc *FTGraphicContext) ClearRect(x1, y1, x2, y2 int) {
...
func (gc *FTGraphicContext) DrawImage(img image.Image) {
	DrawImage(img, gc.img, gc.Current.Tr, draw.Over, gc.Filter)
}

// FillString draws the text at point (0, 0)
func (gc *FTGraphicContext) FillString(text string) (width float64) {
	return gc.FillStringAt(text, 0, 0)
}

// FillStringAt draws the text at the specified point (x, y)
func (gc *FTGraphicContext) FillStringAt(text string, x, y float64) (width float64) {
...
func (gc *FTGraphicContext) StrokeString(text string) (width float64) {
	return gc.StrokeStringAt(text, 0, 0)
}

// StrokeStringAt draws the contour of the text at point (x, y)
func (gc *FTGraphicContext) StrokeStringAt(text string, x, y float64) (width float64) {
	f, err := gc.loadCurrentFont()
	if err != nil {
		log.Println(err)
		return 0.0
...
func (gc *FTGraphicContext) loadCurrentFont() (*truetype.Font, error) {
...
func (gc *FTGraphicContext) drawGlyph(glyph truetype.Index, dx, dy float64) error {
...
func (gc *FTGraphicContext) CreateStringPath(s string, x, y float64) float64 {
	f, err := gc.loadCurrentFont()
	if err != nil {
		log.Println(err)
		return 0.0
...
func (gc *FTGraphicContext) GetStringBounds(s string) (left, top, right, bottom float64) {
	f, err := gc.loadCurrentFont()
	if err != nil {
		log.Println(err)
		return 0, 0, 0, 0
...
func (gc *FTGraphicContext) recalc() {
	gc.Current.Scale = gc.Current.FontSize * float64(gc.DPI) * (64.0 / 72.0)
}

// SetFilter sets the ImageFilter to use for transformations
func (gc *FTGraphicContext) SetFilter(filter ImageFilter) {
	gc.Filter = filter
}

// SetDPI sets the screen resolution in dots per inch.
func (gc *FTGraphicContext) SetDPI(dpi int) {
	gc.DPI = dpi
	gc.recalc()
}

// SetFont sets the font used to draw text.
func (gc *FTGraphicContext) SetFont(font *truetype.Font) {
	gc.Current.Font = font
}

// SetFontSize sets the font size in points (as in ``a 12 point font'').
func (gc *FTGraphicContext) SetFontSize(fontSize float64) {
	gc.Current.FontSize = fontSize
	gc.recalc()
}

func (gc *FTGraphicContext) paint(rasterizer *raster.Rasterizer, color color.Color) {
...
func (gc *FTGraphicContext) Stroke(paths ...*Path) {
	paths = append(paths, gc.Current.Path)
	gc.strokeRasterizer.UseNonZeroWinding = true

	stroker := NewLineStroker(gc.Current.Cap, gc.Current.Join, Transformer{Tr: gc.Current.Tr, Flattener: FtLineBuilder{Adder: gc.strokeRasterizer}})
	...
	for _, p := range paths {
		Flatten(p, liner, gc.Current.Tr.GetScale())
	}
...
func (gc *FTGraphicContext) Fill(paths ...*Path) {
	paths = append(paths, gc.Current.Path)
	gc.fillRasterizer.UseNonZeroWinding = gc.Current.FillRule == FillRuleWinding

	/**** first method ****/
	flattener := Transformer{Tr: gc.Current.Tr, Flattener: FtLineBuilder{Adder: gc.fillRasterizer}}
	for _, p := range paths {
...
func (gc *FTGraphicContext) FillStroke(paths ...*Path) {
	paths = append(paths, gc.Current.Path)
	gc.fillRasterizer.UseNonZeroWinding = gc.Current.FillRule == FillRuleWinding
	gc.strokeRasterizer.UseNonZeroWinding = true
