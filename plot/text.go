...
// NewTextGraphics creates a TextGraphic of dimensions w x h.
...
func (g *TextGraphics) Options() PlotOptions {
	return nil
}
...
func (g *TextGraphics) Path(x, y []int, style Style) {
	GenericPath(g, x, y, style)
}

func (g *TextGraphics) Wedge(x, y, ro, ri int, phi, psi float64, style Style) {
	GenericWedge(g, x, y, ro, ri, phi, psi, CircleStretchFactor, style)
}
...
func (g *TextGraphics) Symbol(x, y int, style Style) {
	g.tb.Put(x, y, rune(style.Symbol))
}

func (g *TextGraphics) XAxis(xrange Range, y, y1 int, options PlotOptions) {
...
func (g *TextGraphics) YAxis(yrange Range, x, x1 int, options PlotOptions) {
	label := yrange.Label
...
func (g *TextGraphics) Key(x, y int, key Key, options PlotOptions) {
	m := key.Place()
	if len(m) == 0 {
		return
	}
	tw, th, cw, rh := key.Layout(g, m, ElementStyle(options, KeyElement).Font)
		...
		for ri, e := range col {
			...
			if plotStyle == -1 {
				...
				if (plotStyle & PlotStylePoints) != 0 {
					g.Symbol(x+int(KeySymbolWidth/2), yy, e.Style)
				}
				if (plotStyle & PlotStyleBox) != 0 {
					g.tb.Put(x+int(KeySymbolWidth/2), yy, rune(e.Style.Symbol))
				}
...
func (g *TextGraphics) Rings(wedges []Wedgeinfo, x, y, ro, ri int) {
	if g.xoff == -1 {
	...
	GenericRings(g, wedges, x+g.xoff, y, ro, ri, 1.8)
}

// var _ Graphics = &TextGraphics{}
