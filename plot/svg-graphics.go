...
func NewSvgGraphics(sp *SVG, width, height int, font string, fontsize int, background color.RGBA) *SvgGraphics {
	if font == "" {
		font = "Helvetica"
	}
	if fontsize == 0 {
...
func AddToSvg(sp *SVG, x, y, width, height int, font string, fontsize int, background color.RGBA) *SvgGraphics {
	s := NewSvgGraphics(sp, width, height, font, fontsize, background)
	s.tx, s.ty = x, y
	return s
}

func (sg *SvgGraphics) Options() PlotOptions {
	return nil
}
...
func (sg *SvgGraphics) Symbol(x, y int, style Style) {
	st := ""
	filled := "fill:solid"
	empty := "fill:none"
...
func (sg *SvgGraphics) Path(x, y []int, style Style) {
	n := len(x)
	if len(y) < n {
		n = len(y)
	}
...
func (sg *SvgGraphics) Wedge(x, y, ro, ri int, phi, psi float64, style Style) {
	panic("No Wedge() for SvgGraphics.")
}

func (sg *SvgGraphics) XAxis(xr Range, ys, yms int, options PlotOptions) {
	GenericXAxis(sg, xr, ys, yms, options)
}
func (sg *SvgGraphics) YAxis(yr Range, xs, xms int, options PlotOptions) {
	GenericYAxis(sg, yr, xs, xms, options)
}

func linestyle(style Style) (s string) {
	lw := style.LineWidth
...
func (sg *SvgGraphics) Key(x, y int, key Key, options PlotOptions) {
	GenericKey(sg, x, y, key, options)
}
...
func (sg *SvgGraphics) Rings(wedges []Wedgeinfo, x, y, ro, ri int) {
