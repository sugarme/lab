...
import (
	...
	ldraw "github.com/sugarme/lab/draw"
...
type ImageGraphics struct {
	...
	w, h   int
	bg     color.RGBA
	gc     ldraw.GraphicContext
	font   *truetype.Font
	fs     map[FontSize]float64
}

// NewImageGraphics creates a new ImageGraphics including an image.RGBA of dimension w x h
// with background bgcol. If font is nil it will use a builtin font.
// If fontsize is empty useful default are used.
func NewImageGraphics(width, height int, bgcol color.RGBA, font *truetype.Font, fontsize map[FontSize]float64) *ImageGraphics {
	img := image.NewRGBA(image.Rect(0, 0, width, height))
	gc := ldraw.NewGraphicContext(img)
	gc.SetLineJoin(ldraw.BevelJoin)
	gc.SetLineCap(ldraw.SquareCap)
...
func AddTo(img *image.RGBA, x, y, width, height int, bgcol color.RGBA, font *truetype.Font, fontsize map[FontSize]float64) *ImageGraphics {
	gc := ldraw.NewGraphicContext(img)
	gc.SetLineJoin(ldraw.BevelJoin)
	gc.SetLineCap(ldraw.SquareCap)
	gc.SetStrokeColor(image.Black)
...
func (ig *ImageGraphics) Options() PlotOptions { return nil }
...
func (ig *ImageGraphics) FontMetrics(font Font) (fw float32, fh int, mono bool) {
...
func (ig *ImageGraphics) TextLen(s string, font Font) int {
...
func (ig *ImageGraphics) setStyle(style Style) {
...
func (ig *ImageGraphics) Line(x0, y0, x1, y1 int, style Style) {
...
var dashPattern map[LineStyle][]float64 = map[LineStyle][]float64{
	SolidLine:      nil, // []float64{10},
	DashedLine:     []float64{10, 4},
	DottedLine:     []float64{4, 3},
	DashDotDotLine: []float64{10, 3, 3, 3, 3, 3},
	LongDashLine:   []float64{10, 8},
	LongDotLine:    []float64{4, 8},
}

func (ig *ImageGraphics) Path(x, y []int, style Style) {
	ig.setStyle(style)
...
func (ig *ImageGraphics) relFontsizeToPixel(rel FontSize) float64 {
...
func ConstructFontSizes(basesize float64) map[FontSize]float64 {
	size := make(map[FontSize]float64)
	for rs := int(TinyFontSize); rs <= int(HugeFontSize); rs++ {
		size[FontSize(rs)] = basesize * math.Pow(1.2, float64(rs))
	}
	return size
}

func (ig *ImageGraphics) Text(x, y int, t string, align string, rot int, f Font) {
	if len(align) == 1 {
		align = "c" + align
	}
...
func (ig *ImageGraphics) textBox(t string, font Font, textCol color.Color) image.Image {
...
func (ig *ImageGraphics) Symbol(x, y int, style Style) {
	GenericSymbol(ig, x, y, style)
}

func (ig *ImageGraphics) Rect(x, y, w, h int, style Style) {
	ig.setStyle(style)
...
func (ig *ImageGraphics) Wedge(ix, iy, iro, iri int, phi, psi float64, style Style) {
	ig.setStyle(style)
	stroke := func() { ig.gc.Stroke() }
	if style.FillColor != nil {
		ig.gc.SetFillColor(style.FillColor)
...
func (ig *ImageGraphics) XAxis(xr Range, ys, yms int, options PlotOptions) {
	GenericXAxis(ig, xr, ys, yms, options)
}
func (ig *ImageGraphics) YAxis(yr Range, xs, xms int, options PlotOptions) {
	GenericYAxis(ig, yr, xs, xms, options)
}

func (ig *ImageGraphics) Scatter(points []EPoint, plotstyle PlotStyle, style Style) {
	GenericScatter(ig, points, plotstyle, style)
}

func (ig *ImageGraphics) Boxes(boxes []Box, width int, style Style) {
	GenericBoxes(ig, boxes, width, style)
}

func (ig *ImageGraphics) Key(x, y int, key Key, options PlotOptions) {
	GenericKey(ig, x, y, key, options)
}

func (ig *ImageGraphics) Bars(bars []Barinfo, style Style) {
	GenericBars(ig, bars, style)
}

func (ig *ImageGraphics) Rings(wedges []Wedgeinfo, x, y, ro, ri int) {
	GenericRings(ig, wedges, x, y, ro, ri, 1)
}
